...
export const addReaction = async (req, res) => {
    try {
        const modThought = await Thought.findOneAndUpdate({ _id: req.params.thoughtId }, {
            $push: {
                reactions: {
...
export const deleteReaction = async (req, res) => {
    try {
        const modThought = await Thought.findOneAndUpdate({ _id: req.params.thoughtId }, { $pull: { reactions: { reactionId: req.params.reactionId } } }, { new: true });
        if (!modThought) {
            res.status(404).json({ message: 'Could not find thought' });
            return;
        }
